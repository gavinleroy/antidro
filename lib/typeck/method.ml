...
let resolve_application gamma fty ids tys err cont =
  let is_signature_applicable (sign : polysig) =
    unify_candidate sign ids tys
    |> Option.map (fun _ -> true)
    |> Option.value ~default:false
  in
  match fty with
  | FnT signature ->
      if is_signature_applicable signature then cont (`LambdaApp signature)
      else err `BadApplication
  | MethodGroupT groupid -> (
      let ms = Gamma.methods groupid gamma in
      ms
      |> List.filter (fun m -> is_signature_applicable m.signature)
      |> function
      | [] ->
